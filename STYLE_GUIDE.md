...
Note: please raise an issue if you spot a place where anything contradicts this guide (regardless of the author). Either there will be an issue discussing it, or it is a new (thus valuable) contribution.
...
Quotation is preferred when practical. References (links to a source) are ideally reserved for insignificant matters (e.g. a link in a footnote to some source/evidence) and broad matters (such as a system that someone has put forward, or a topic / field of study). Notably: quotes should also contain references or be referenced prior -- a reader must always be able to tell the exact source of a quote and have access to verify it.

**QUOTES MUST BE EXACT AND PROPER -- ZERO TOLERANCE FOR MISQUOTATIONS**

When you quote someone, you are attributing something to them in a precise way. Misquotes are fake-quotes that *look* like real quotes. This includes improper use of `"` (quotation marks), typographical errors (or worse, like changing words) errors in recording the quote, fabrications (v bad), and *dishonest* quotations (which are *technically* accurate, except that there is *context-dropping* or *any other method of misrepresenting the author*.) It doesn't matter if you're quoting Hitler. *Quote accurately, quote honestly. No excuses.*
...
References to uncontrolled resources should only be made to those resources which are *extremely* reliable (i.e., are almost-certain to exist in ~20 years time -- like archive.org or github.com). If there is any doubt, then use a service like IPFS or archive.org to create a verifiable and persistently accessible version. When referring to a paper, IPFS is preferable, along with redundant links (e.g., to arXiv or similar). More links = good. In general, IPFS is acceptable if you have reasonable confidence that a version will be maintained -- for this reason multiple IPFS nodes will be maintained that pin crucial documents. IPFS is preferred for single files because it is cryptographically verifiable and does not rely on us (so other ppl can trust it without trusting us).

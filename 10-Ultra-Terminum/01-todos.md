\todoDraftOnly{explain header size: 112 = 80 + 32 (btc efficiency + an extra parent to support DAG data struct)}

\todoDraftOnly{discuss impact of PoW/PoS choice for simplex-/dapp-chains -- PoS overhead (sorta discussed atm, but only for headers)}

\todoDraftOnly{por-sim-rs: dedup reflections -- need to do that? (e.g. via dag); double counting a small number of refls probs makes simplexes a little more secure? effect should increase with v small block period?}

\todoDraftOnly{revisit PoR sections -- pors don't need to be for best block}



%% END ### RELEASE

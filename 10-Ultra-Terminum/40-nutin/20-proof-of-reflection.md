...
Let's also add variables for Chain \cR, $R_f$, $R_r$, and $R_d$, which correspond to those we already have for Chain \cL.
...
A ratio is found by multiplying the previous difficulty ($D_\text{prev}$) by the target duration ($\Delta t_\text{target}$) for 2016 blocks and dividing by the actual duration ($\Delta t_\text{actual}$) of the last 2016 blocks.\footnote{

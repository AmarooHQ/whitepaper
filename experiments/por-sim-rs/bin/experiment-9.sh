...
# exp 9: does using DoubleSpendWork fix?
...
for repeat_i in `seq 1 ${REPEAT_TIMES}`; do
  ...
  for atk_q in 0.40; do # 0.44; do
    for ds_confs in 20; do # 10 20; do # 5 10 20; do
      export ATK_RATIO=${atk_q}
      export ATK_DS_CONFS=${ds_confs}
      export OUT_FILE=exp-9-R${ATK_STRATEGY}-q${ATK_RATIO}-t${ATK_DS_CONFS}-p${B_PERIOD}-H${HR_PER_CHAIN}-${CRYPTO_SYSTEM}-blake3.csv
      if [[ ! -f $OUT_FILE ]]; then

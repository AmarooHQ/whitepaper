...
\subsection{Bandwidth Complexity}\label{bandwidth-complexity}}
...
\pz{Unclear why with explicit PoRs the complexity is $O(c)$ and not $O(c \cdot \log_2 c)$ because we just defined the sizes of PoRs as $O(N_1 \log_2 N_1)$.}
...
Practically, $\Delta r$ looks to be between $k_1$ and $3 k_1$ for $k_1=3000$ --- but, this is sensitive to $\phi$.
...
\end{align*}
If $k_1 = 3000$, $\phi = 1$, we have $B_h \ge 235$ --- much larger than what we've assumed to be the minimum so far.
...
To keep $\Delta r$ practicable, we should therefore go to some effort to keep $g k_1 \phi \lesssim 2 {B_h}^2 z$ (where $z$ is a small constant we can use to add some acceptable tolerance in lieu of improving $\phi$).
...
\end{align}
A maximal simplex of reasonable parameters has $\Delta r \approx 7\text{,}200$ B/s.
\pz{Unclear how restricting bandwidth requirements creates a lower bound on $B_h$, we'd rather expect an upper bound.}
Restricting $\Delta r < k_1$ now implies $k_1 < 2 {B_h}^2 (g \phi)^{-1}$.
If $k_1 = 3000$, $\phi = 1$, we have $B_h \ge 219$.


\subsubsection{Complete Simplex}

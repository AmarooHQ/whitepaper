...
To keep $\Delta r$ practicable, we should therefore go to some effort to keep $g k_1 \phi \lesssim 2 {B_h}^2 z$ (where $z$ is a constant near 1 that we can use to add some acceptable tolerance in lieu of improving $\phi$).
...
\begin{equation}
\begin{split}
\Delta S &\le N_1 \cdot k_1 = \frac{{k_1}^2}{4 \cdot B_f \cdot B_h} \nonumber \\
\implies O(\Delta S) &= O(c^2) \nonumber
\end{split}
\end{equation}
It is clear that \(\Delta S\) has order \(O(c^2)\), but how bad is this?
For \(k_1 = 3000\), \(B_f = \nicefrac{1}{60}\), and \(B_h = 112\): \(\Delta S \approx 1.2\) MB/s.

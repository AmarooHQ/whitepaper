...
For \UT{\text{+OP}}, the data a full node requires are: each block, the headers of all reflecting chains, and the missing branches for all
PoRs.
...
\todoDraftOnly{in effect: we get to choose the aspect that the $\log c$ influences -- this matters b/c of \emph{a principle of scaling} section.
we get to choose based on *worst case* so basically scalability isn't affected by that term b/c we can always put it in the non-bottleneck component.}
...
Let's call the total bandwidth requirements for the PoR graph $\Delta r$.
\begin{align}{}
...
To keep $\Delta r$ practicable, we should therefore go to some effort to keep $g k_1 \phi \ll 2 {B_h}^2$.

\subsubsubsection{Complete Simplex}

\aside{
    This is also the bandwidth required by all miners due to the \AxiomOfAvailability.

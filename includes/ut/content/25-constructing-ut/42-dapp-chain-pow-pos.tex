...
\subparagraph{Spam, Availability, and Dapp-chains: A PoW/PoS Asymmetry}
...
\aside{
  Note that \emph{Proofs of Availability} are of no use here, because verifying them requires at least as many bytes as the block itself\footnote{
...
First, it's important to note that within the ``Proof of Stake'' family of consensus methods, there are many variants.
...
\aside{
  ...
  So, if any contemporary PoS chains are secure (putting aside nothing-at-stake and long-range attacks, since the PoW context of the simplex solves those), then we'll have a viable option for PoS dapp-chains.
}

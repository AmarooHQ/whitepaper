\subsubsection{\titlemath{$\UT{1}$}{UT₁}: Scaling Complexity Intuition}

\aside{
    Although we will rigorously investigate \UT{}'s scaling complexity in \autoref{sec:scaling-complexity}, it will prove useful to introduce $N_1$ here and to gain an intuition for UT's scaling complexity before \autoref{sec:practical-considerations}.
}
...
So, there must be a capacity maxima between 2 simplex-chains and the absolute maximum.

What we're observing is that the overall capacity is the \emph{product} of size of the simplex and each chain's remaining capacity.
...
However, the \emph{network-wide} capacity is the \emph{product} of $t$ and $r$.
Maximizing $tr$ suggests that a chain's capacity should be split approximately 50/50 between $t$ and $r$, so $t=r=\half=O(1)$.
Given that $t$ and $r$ are proportions of the capacity, we can say $tO(c) + rO(c) = O(c)$.
Thus, the overall capacity is given by $tO(c) \cdot rO(c) = O(c^2)$.
...
Let's define $N_1$ as the number of chains which maximizes a simplex's capacity (for transactions).

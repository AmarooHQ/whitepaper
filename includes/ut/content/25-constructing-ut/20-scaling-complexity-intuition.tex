...
\aside{
    Although we will rigorously investigate \UT{}'s scaling complexity in \Cref{sec:ut-complexity}, it will prove useful to introduce $N_1$ here and to gain an intuition for UT's scalability before \Cref{sec:practical-considerations}.
}
...
\mk{Change back to $t$ and $r$ if no good reason to change them. If there is a good reason, $\Theta_t$ is kinda ugly. we can probably pick a better symbol.}

Let's call the proportion of capacity for transactions $\Theta_t$, and the proportion for reflections $\Theta_r$.
We know that $\Theta_t + \Theta_r = 1$, since $\Theta_t+\Theta_r$ is 100\% of a chain's capacity.
However, the \emph{network-wide} capacity is proportional to the \emph{product} of $\Theta_t$ and $\Theta_r$.
Maximizing $\Theta_t \Theta_r$ suggests that a chain's capacity should be split approximately 50/50 between $\Theta_t$ and $\Theta_r$, so $\Theta_t=\Theta_r=\half=O(1)$.
Given that $\Theta_t$ and $\Theta_r$ are proportions of the capacity, we can say $\Theta_t O(c) + \Theta_r O(c) = O(c)$.
Thus, the overall capacity is given by $\Theta_t O(c) \cdot \Theta_r O(c) = O(c^2)$.

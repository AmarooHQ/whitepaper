...
In practice the `pull' requires the user publish a proof about the sending chain to the receiving chain.
...
The delay duration itself is almost unconstrained: fraud proofs propagate well within one block period (at which point honest nodes are aware of them), and are included in the PoR graph within a block period, too.
However, if we pick too short a period then we risk some fragility in the network.
We can technically recover if an invalid cross-chain transaction were processed before a corresponding fraud proof was recorded, but doing so robs honest miners of their block reward.
...
There is plenty of time for the fraud proof to propagate and be recorded, and minimal risk of a miner's block being invalidated.
...
We can now summarize; when drafting a block, blocks which are valid for cross-chain processing are those which:
\begin{enumerate}
    ...
    \item \label{e-xc-cond3} Are in the history of this block's $\lambda$-parent\footnote{
...
The protocol for processing cross-chain transactions is now, simply: when drafting a block, miners gather all blocks which are valid for cross-chain processing, scan them for incoming cross-chain outputs, and include those cross-chain outputs in the draft block.

\aside{
    In reality, this isn't ideal because updating the reflections of a draft block may introduce new blocks which are valid for cross-chain processing.
...
If one did want to craft an explicit proof $\pi$, then it would be similar in size to a normal transaction proof (assuming headers in the surrounding PoR graph are available).
...
If you wish to make a local transaction, it could take a much longer than a block period.
An alternative is to make a cross-chain transaction instead.
...
You could then `rescue' your coins and send them to a more useful chain.\footnote{
    ...
    Since we must scan blocks as part of the cross-chain processing, the branch taken should also be recorded in the block, along-side the corresponding output or transaction.
}

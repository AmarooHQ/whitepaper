...
However, if blocks are allowed to have \emph{more} than one parent then there \emph{is no point} where an attacker can \emph{maintain} a DoS attack indefinitely. Instead, they can only \emph{delay the execution} of some transactions for a short period of time.

...
\subsubsection{Block-DAGs Prevent DoS Attacks}
...
\begin{figure}[ht]
    ...
    \caption[
        ...
        An attempted empty-block DoS attack on a block-DAG.
        The attacker's blocks, $A_{i+x}$, are mined in public and contain no transactions.
...
If an attacker performs a \emph{repeating cycle} of these attacks, then it may be able to decrease the effective capacity of the chain by a factor of $\sim {A_{\text{blocks}}}^2$. %<!-- / (1 + A_{\text{blocks}}) -->
The opportunity cost of this attack, for the attacker, is at least as much as the lost transaction fees.

We'll discuss DoS attacks again, shortly, in \autoref{sec:dags-and-the-por-graph}.

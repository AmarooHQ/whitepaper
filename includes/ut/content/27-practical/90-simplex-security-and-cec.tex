...
\begin{itemize}
    \item attacker q (e.g., 0.44) divisibility into B\_PERIOD and/or HR\_PER\_CHAIN
    \item small HR\_PER\_CHAIN -- only 50-100 hashes every tick
    \item small values $\to$ limits of accuracy mb? (in other ways, not just attacker q divisibility)
    \item using xxh3 instead of a crypto secure hashing alg like blake3
    \item not giving the attacker a 'bonus block' (i.e. they only start after having mined a block in private)
    \item not waiting long enough for DSs to resolve (since we end before $t=\infty$ it is the case we cut off \emph{some} DSs that might be successful -- is it significant?)

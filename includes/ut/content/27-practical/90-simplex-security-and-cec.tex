...
\defineTermTex{Confirmation Equivalence Conjecture (CEC)}{
    ...
    % $\forall a \in \{1,2,\dots,N\}: P(q; N_1 = a; c = \frac{NC}{a})$
}
...
Define $P(q; N_1 = N; c = C)$ (for some given $N$ and $C$) to be the probability of an attack succeeding after $c$ confirmations against a simplex with $N_1$ simplex-chains, given an attacker with $q$ proportion of the network hash-rate.

Note that a simplex of 1 chain (the 0-simplex) \emph{is} a traditional blockchain.
So we can take Bitcoin for example: the probability of an attack succeeding after 6 confirmations is given by $P(q; N_1 = 1; c = 6)$ -- of course, we still need to know the attacker's hash-rate, $q$, to calculate this.

Thus, the hypothesis is that:
\begin{equation}
    P(q; N_1 = 1; c = NC) \approx P(q; N_1 = N; c = C)
...
So, we don't need to know a closed form of $P(q, N_1, c)$: all cases \emph{that we can measure} are equivalent to some \emph{other} case, and we can convert between them.\footnote{
...
Note that $P(q; N_1 = 1; c = C)$ is taken to be identical to Rosenfeld's analytical solution. That is:

\begin{equation}
    \forall c \in \{1, 2, \dots\}: P(q; N_1 = 1; c) = \begin{cases}
...
Graph 1:
PoR: $P(q; N_1 = x; c = C)$ vs Traditional: $P(q; N_1 = 1; c = Cx)$

Graph 2:
Simplex A vs Simplex B
\begin{align*}
    \text{Simplex A} \to & & P(q; N_1 = x / 2; c = C) &\approx P(q; N_1 = x; c = 2C) & & \leftarrow \text{Simplex B} \\
    & & P(q; N_1 = x; c = C) &\approx P(q; N_1 = 2x; c = 2C) & &
\end{align*}
...
Following the initial implementation of PoR in the simulator, results were promising but did not align with the theoretical prediction (based on Rosenfeld's analytical derivation of $P(q; N_1 = 1; c = C)$).

traditional chains were okay but not perfect -- a bit better than predicted tho.
simplexes, however, were way worse for $N_1 > 5$ or so -- $P(\cdots)$ never dropped below $\sim 0.1$.
...
\begin{itemize}
    \item benchmark against traditional blockchain analytical solution (Rosenfeld, 2012): $P(q; N_1 = 1; c = Cx)$
    \item plot of measured $P(q; N_1 = 1; c = Cx)$
    \item plot of measured $P(q; N_1 = x; c = C)$
    \item plot of scaled measured $P(q; N_1 = x/a; c = Ca)$ for different values of $a$

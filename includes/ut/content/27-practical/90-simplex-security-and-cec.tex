...
The attacker should only be able to perform control the network's history if $q > p$.
...
So, we don't need to know a closed form of $P(q, N_1, c)$: all cases \emph{that we can measure} are equivalent to some \emph{other} case, and we can convert between them.\footnote{
    ...
    However, this is because the solution assumes a static difficulty; this works for Bitcoin, but not for chains that use faster-adjusting DAAs.
    When using \emph{work} instead of \emph{height} as the measure of an attack's success, fractional values of $c$ have real meaning, especially with a reactive DAA and $N_1 \gg 1$.
    This turns out not to be much of an issue for us, here, though.
}
...
These results are, for the purpose of this paper, taken to indicate the following statements are true.\footnote{
    ...
    For example: the fact that these experimental results converge as expected is NOT evidence that UT is secure, or that PoR works, or that the CEC is right -- however, they are \emph{criticisms} of ideas like: \emph{PoR is broken}, or \emph{Simplexes aren't secure}, or \emph{Sharing security between blockchains is impossible}.
    For any of those criticisms (of UT) to be correct, there must be some \emph{counter-refutation} of these results.
    That could be an explanation of: the implementation was flawed, the methodology was flawed, or that UT cannot be effectively implemented as a production blockchain.
    In the \emph{absence} of such criticisms, it is reasonable to work under the assumption that UT is secure, since that is currently the only viable explanation for these results.
}
...
\begin{enumerate}
    ...
    \item Decreasing $DAA_N$ increases the security of both simplexes and traditional blockchains.
    ...
    \item Rosenfeld's closed form solution is consistent with empirical evidence.
\end{enumerate}

...
For any $a,b \in \{1, 2, \dots, N\}$, the CEC implies:
\begin{equation}
...
\begin{table}[H]
    ...
    \caption[
        ...
        Table of parameters necessary to generate experimental results.
        Some parameters as they are not directly relevant to this experiment.
    }
    \begin{tabular}{lp{0.7\linewidth}}
        \toprule
        Parameter & Significance \\
        \midrule
        $q$ (ratio) & Attacker's proportion of global hash rate. \\
        $c$ (confirmations) & Local confirmations required before the attacker can succeed. \\
        $B_f^{-1}$ (ticks) & Analogue of ``seconds'' used by the DAA. \\
        $DAA_N$ (blocks) & The number of blocks over which the DAA operates. \\
        $N_1$ (chains) & The number simplex-chains ($N_1 = 1$ for traditional blockchains). \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[]
    ...
    \caption[]{
        2 A comparison of the measured probabilities of a successful doublespend (for a traditional blockchain and for a simplex) compared with Rosenfeld's analytical solution, for $q \in \{0.4, 0.44\}$.
    }
...
\begin{figure}[]
    ...
    \caption[]{
        3 A comparison of the measured probability of a successful doublespend compared with Rosenfeld's analytical solution, for $q \in \{0.4, 0.44\}$.
    }

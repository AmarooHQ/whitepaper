...
\subsubsubsection{Basic Structure}\label{basic-structure}}
...
Often, the motivation for using a block-DAG -- instead of a block-tree
-- is to increase the block frequency.
...
Slower block frequencies also decrease the incidence of multiple parents
-- this matters because each parent typically increases the header size by 32 bytes.

Some basic block-dag segments are shown in
\autoref{fig:dag-simple-segments}.
...
The essence of DAG-based consensus (at least the kind we're concerned with) is to \emph{prioritize execution} of blocks and transactions based on the \emph{security contribution} (i.e., weight) of each parent block
...
\footnotetext{
    ...
    Provided that parent blocks \emph{are prioritized by cumulative work}, the chain of \emph{primary ancestors} between a given  and the genesis block must be the \emph{single heaviest path} between the two.
    (This is the case for UT.)
    (TODO: FACT CHECK)
}
...
The subgraph which takes priority is that of the
\emph{prioritized parent's ancestry}.

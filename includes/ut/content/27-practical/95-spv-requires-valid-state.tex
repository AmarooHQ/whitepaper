...
\rMax{
    Since the attacker loses the actual block rewards, too, the total cost after $k$ blocks is the $k$-th triangle number $+ k$.
    ish. That's $O(k^2)$ cost.
}
\rSe{
...
\rMax{
    Maybe -- only if that later block looks like the best block.
    Like, the goal of such a miner is to have that later block be favored by the fork rule.
    That shouldn't happen with the way that we count work, though.
    Like, if the later block uses any PoRs via R-blocks that reflect the earlier block, then the work will be attributed to the earlier block rather than the common ancestor.
}
\rMax{
    I have just thought of a way to tighten things up, though.
    Consider that miners must only reflect blocks that they've downloaded, and that condition is recursive.
    So, if a new L-block does PoR via some R-block, and that R-block reflects an L-block that isn't in the main L-block's history -- well that implies that the miner has downloaded that other L-block but has not linked back to it.
    But a miner can link back to both valid and invalid blocks.
    If the other L-block is valid, then then honest miner should build on it, and if it's invalid then they should flag it as invalid.
}
\rMax{
    Is there ever a case where it's honest to ignore a block like that? \\
    I suspect not.
}


\rMax{
    Don't do consensus on state-transitions. \\
    \\
    Do consensus on data availability, maximal interconnection, and error correction of state-transitions.
}

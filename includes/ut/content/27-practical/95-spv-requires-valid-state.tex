...
% \assignedTODO{{Max > Pouriya}}{h}{n/c adversary --- explain it (Max to dot-point/draft, Pouriya to writeup)}
...
\todoDraftOnly[h]{Check that we cover safe and reliable explicitly (or add)}
\dSe{
...
\pz{H is used earlier as the total hash-rate of the network. Also $z$ should be defined.}
\dMax{
...
\begin{itemize}
    ...
    \item Each round, the attacker must pay $C_r Z_k$ in bribes (where $Z$ is the bribe, and $k$ is the round, starting at 1).
...
\cn{Re-introduce the batching from the error correction. ``Recall in our dialog we talked about waiting for SPV transactions and executing them in batches \dots''}
...
\mk{Should highlight this more, important result}

...
\dMax{
    First, a process one: \emph{non-statistical analysis is good enough} -- at least for now.
}
\dSe{
...
\dMax{
    Because the attacker wants to prevent the FP being recorded in the PoR graph, and the FP is readily available.
}
\dMax{
...
% also: (thought of after) that going from invalid -> valid histories is a reorg at all.
\dSe{
...
\dMax{
    So we actually \emph{do} want \emph{some} future blocks to be able to invalidate past blocks.
    The ones that honest miners make.
}
\dSe{
...
\dMax{
    And premise 2 is false, too, because basically all nodes know of the FP, which includes honest miners.
}
\dSe{
    So all three were false.
}
\dSe{
...
\dMax{
    ...
    More even -- they're based on ideas that I learnt years ago.
}
\dSe{
...
\dMax{
    For sibling chain-tips, the reorg isn't very disruptive and happens somewhat regularly.
    But deeper reorgs can be disruptive because state can change significantly -- that's how doublespends are done.
}
\dSe{
    And when a fraud proof is added to the PoR graph, that causes a reorg?
}
\dMax{
    Yeah.
}
...
\dMax{
    If a totally different chain, like Eth2, was verifying SPV proofs against the simplex, it must be aware of unpublished fraud proofs to be secure.
...
\dMax{
    Yeah, okay.
    So\dots{} what does refuting the premise mean for SPV, then?
}
\dSe{
...
\dSe{
    Do you have any reason to think that we can't reduce the mandatory SPV delay?
}
\dMax{
    \dots{} I guess not.
}
...
\dMax{
    You could \emph{make} the transaction at that point, but a miner shouldn't include it, yet.
}
\dMax{
    An honest miner \emph{should} almost always hear about the FP before the next block, though.
...
\dMax{
    So we should wait at least 1 confirmation, but small reorgs are possible, so we need a little buffer, too.
}
\dSe{
...
\dMax{
    ...
    But eventually, any inconsistency could cause the miners on the smaller partition(s) lose out.
}
\dMax{
    Actaully, no fraud proof would be possible (assuming honest miners).
}
\dMax{
    All of the SPV transactions would become invalid when the partitions merge, because they target headers that are no longer in the respective main chains.
    So they'd be invalid when replayed.
}
\dMax{
    But if users know to be cautious, is that really an issue?
}
...
\dMax{
    ...
    Each block has some capacity for reflections, and we know the average number of reflections per block.
...
\dMax{
    So if we had $3\times$ the average -- such that two-thirds is reserved as \emph{excess capacity} -- then we can handle those high volume times, and otherwise we can integrate reflections twice as fast as the average, without bottle-necking the production of new blocks.
}
\dMax{
...
\dMax{
    Although, the main chain will gain work up to $3\times$ faster than before the partition merged.
}
\dMax{
...
\tikzstyle{n} = [stdText, draw, rectangle, rounded corners]
\tikzstyle{ar} = [->, anchor=north, rounded corners, shorten >= 2pt, shorten <= 2pt]
\tikzstyle{label} = [align=center, anchor=center]
\tikzstyle{l2} = [align=left, anchor=west, xshift=-1.5em]
\tikzstyle{lb} = [align=center, anchor=north, yshift=-1em]
\tikzstyle{braceUnder} = [decorate, decoration={brace,amplitude=6pt,mirror,raise=2pt}]%
\begin{center}
    \begin{tikzpicture}[node distance=.8cm]
        % the conditions
        \node (f1) [n] {Check \\ Header};
        \node (f2) [n, below right=of f1] {Check \\ Form};
        \node (f3) [n, below right=of f2] {Check \\ PoRs};
        \node (f4) [n, below right=of f3] {Check \\ State};
        \node (f3a) at (f3 |- f4) {};
        \node (f5) [n, below left=of f4] {Fraud \\ Proof};

        \node (start) [xshift=-1.2em] at ($(f1)!2.3!(f1.north west)$) {};

        \node (a1) [n, left = of f1, xshift=-.5cm] {Junk};
        \node (a2) [n] at (a1 |- f2) {Unreadable; \\ Discard (Junk)};
        \node (a3) [n] at (a1 |- f3) {Invalid; \\ Use w/ FP};
        % \node (a3a) [n] at (a1 |- f3a) {Add to \\ PoR Graph};
        % \node (a4) [n] at (a1 |- f4) {Valid PoRs \\ Link FP if possible};
        \node (a4) [n] at (a1 |- f4) {Add to \\ PoR Graph};
        \node (a5) [n] at (a1 |- f5) {Add FP to \\ PoR Graph};
        ...
        \node (newBlock) [label, draw, sharp corners, dashed] at (start -| actions) {Receive \\ New Block};

        \draw [ar] (newBlock) -- (start -| f1) -- (f1);
        \draw [ar] (f1) -- node [l2] {Good PoW \\} (f1 -| f2) -- (f2);
        \draw [ar] (f2) -- node [l2] {Good Form \\} (f2 -| f3) -- (f3);
        \draw [ar] (f3) -- node [label, anchor=east] {Valid \\ PoRs} (f3 |- f4) -- (a4);
        \draw [ar] (f3) -- (f3 |- f4) -- (f4);
        \draw [ar] (f4) -- node [label, anchor=west, align=left] {Fail} (f4 |- f5) -- (f5);
        \draw [ar] (f4) -- node [label] {Pass; \\ Done} (fDone);
        ...
        \node (BR) at ($(divPointB)!2!(f4.south |- divPointB)$) {};
        \node (TR) at (BR |- divPointT) {};
        \draw [thick, dashed] (divPointT) -- (divPointB.south);

        \node (BL1) at (BR -| f1.west) {};

        \draw [braceUnder] (BL1) -- node [lb] {$O(c)$} (divPointB);
        \draw [braceUnder] (divPointB) -- node [lb] {$O(c^2)$} (BR);

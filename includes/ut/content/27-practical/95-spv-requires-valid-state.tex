...
\dialogHeader{24\textsuperscript{nd} August 2022}
...
\dSe{
    ...
    Where could an unseen premise be hiding?
}
\dMax{
...
\dMax{
    It's what happens when the chain-tip changes to a second chain-tip that isn't a descendant of the first one.
}
\dMax{
...
\dMax{
    ...
    They had the fraud proof already, so they knew it was invalid.
}
\dSe{
...
\dMax{
    ...
    Or, at least, those are the only ones that matter.
}
\dSe{
    Right.
}
\dMax{
    So\dots{} what does this mean for SPV?
}
\dSe{
    Retrace the conversation, to before any of the premises we just refuted, back to the last unrefuted thing you said.
}
\dSe{
    What was it?
}
...
\dMax{
    I guess we need some minimum waiting period so the FP has time to propagate, but that should be very fast if the P2P layer prioritizes it.
}
\dSe{

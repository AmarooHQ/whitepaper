...
\dSe{
    Now, consider premise 1.
    It's only four words; where could a premise be hiding.
}
\dMax{
...
\dMax{
    ...
    Because it never used the attacker's branch -- that was invalid.
}
\dMax{
    So it's like a typical newly mined block, except it was a long time since its parent.
    And it links back to like potentailly thousands of previous invalid blocks.
}
\dSe{
...
\dSe{
    If a partition split off a long time ago (say, hours) there'd be many thousands of blocks in quick succession.
    Is that a problem?
}
\dMax{
    Uhh, well all of those blocks would get reflected by the rest of the PoR graph -- sequentially.
    Both partitions would have to do that, based on the rules we introduced a few weeks ago.
}
\dMax{

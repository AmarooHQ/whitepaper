...
\dMax{
    And premise 2 is false, too, because basically all nodes know of the FP, which includes honest miners, so can always avoid the catch 22.
}
\dSe{
    So.
...
\dMax{
    So it'd be like a typical new block, except it was a long time since its parent. \\
...
\dMax{
    ...
    The knew the block was invalid because they already had the fraud proof.
}
\dSe{
    So\dots{}
}
...
\dMax{
    You could \emph{make} the SPV tx at that point, but a miner shouldn't include it, yet.
}
\dMax{
...
\dMax{
    ...
    To do that we can check the difference between total chain-weight --- for each SPV transaction, each chain can check: when the header targeted by this SPV tx was reflected by us, is that chain-weight plus $C_d N_1 z$ less than the current chain-weight?
}
\dMax{

...
Therefore, it an attacker must have additional support to maintain this portion of the attack -- when the mining losses per block (approx $C_r H k$) exceed the attacker's mining capacity ($C_r N_1 q$).
% We have:
\begin{align}
    \text{\hspace{8em}}
    && C_r H z &< C_r N_1 q \nonumber \\
...
\todo{nomenclature clash: $h$ used both for $H(Np+q)$ and in $h+d=1$.}
...
\begin{align}
    yz &= \BribeCost(z) = h \frac{z(z+1)}{2} + z + 1
        \nonumber \\
    y &= h \frac{z+1}{2} + 1 + \frac{1}{z}
        \nonumber
\end{align}
This is a hyperbola and, as $z$ increases, it approaches $y = \frac{h}{2}(z+1) + 1$.
Alternatively: $z = \frac{2}{h}(y - 1) - 1$.

So, if $y = 10$ and $h = 1$, then we can estimate that we have a fixed point at $z \approx 17$ blocks.
...
The duration of this transition depends on the proportion of honest miners ($h$, where $h + d = 1$ and $d$ is the proportion of dishonest miners, including the attacker).
...
\begin{center}
    \begin{tikzpicture}[node distance=.8cm]
        ...
        \node (actions) [stdText, above=of a1] {Action};
        \node (p1) [stdText] at (actions -| f1) {P1};
        \node (p2) [stdText] at (actions -| f2) {P2};
        \node (p3) [stdText] at (actions -| f3) {P3};
        \node (p4) [stdText] at (actions -| f4) {P4};
    \end{tikzpicture}
\end{center}

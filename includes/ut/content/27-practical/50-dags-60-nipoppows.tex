...
\subsubsection{Adapted NIPoPoWs for Block-DAGs: NIPoPPoWs}

\label{sec:blockdag-nipopows}
% \label{sec:nipoppows}

\warning{
    Reminder: \autoref{sec:blockdag-nipopows} and sections on NIPoPoWs are speculative.
}
...
(That is: we make sure the necessary on-main blocks are included in the proof if they wouldn't be otherwise.)
Both together comprise the suffix proof.

\pz{Add where \texttt{followDown} and \texttt{verify-infx} are coming from. Maybe "we can use the existing \texttt{followDown} algorithm, used to find\dots" }
Regarding infix proofs, we can use the existing\footnote{Defined in the \citeNIPoPoWs{} paper.} \texttt{followDown} algorithm, is used to navigate between superblocks and regular blocks, with only one modification: an additional parameter determines whether \texttt{followDown} should traverse the \emph{best-blocks} path to the block, or the \emph{on-main only} path.
...
\footnotetext{
    A definition is required as this word is an almost-neologism:

    \textbf{Posteritorial} \emph{adjective}: having posterity; supported by its descendants, especially regarding well-known descendancy, epistemic antecedence, root-causal nature, etc; having a prototypical and defining quality that was widely inherited.

    \emph{Aside:} Perhaps curiously, a prior candidate modifier was the latin phrase \emph{per posterās} --- literally `through what came after'.
    But also faux-translated as `[as] per post eras', which is roughly equivalent to the English phrase `via posterity'.
    A downside of \emph{per posterās} is that, phonetically, it's very similar to `preposterous', and a Non-Interactive Proof of Preposterous Proof of Work is not an apt name.
}

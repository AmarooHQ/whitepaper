\subsubsection{Fraud Proofs \& Bribe Attacks: Derivations \& Breakpoints}

\label{sec:fps-and-bribes}
...
We need to know what a healthy simplex (and its PoR graph) looks like because a successful attack will start off indistinguishable.
...
An \emph{asymmetric boundary} emerges when a group of miners contributes \emph{exclusively} to a \emph{subgraph} of the full PoR graph (i.e., the subgraph that censors the fraud proof).
...
The fraud is `invisible' when considering just the blockchain data itself, which means that SPV proofs and the like may be fraudulent.
...
Compared to mining honestly, the attacker forgoes the reward of $C_r H k$ for the $k^{\text{th}}$ block.
...
Let the total cost to the attacker, in the $k^\text{th}$ round, be $C_r Z_k$ coins.
...
Therefore, at the $k^\text{th}$ round, $k \ge 2$:
\begin{align}
...
If we assume an honest minority of substantial size, $h = \nicefrac{1}{3}$, then $k > 2$.
That is, the honest chain-tips are expected to dominate after $2$ block periods if $\nicefrac{1}{3}$ of miners are honest.

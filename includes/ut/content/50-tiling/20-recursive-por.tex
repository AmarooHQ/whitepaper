...
We combine these two for the full path by which \BR2 implicitly reflects \BL1:
$\xyInlineRefl{\BL1 \;&\; \BM{1}\arf[l] \;&\; \BR{2}\arf[l]}$.

Similarly, we know that \BL{3} implicitly reflects \BR{2}: $\xyInlineRefl{\BR2\;&\;\BM2\arf[l]\;&\;\BL3\arf[l]}$.

Thus, for \BL3, we can construct a full recursive PoR of \BL1 via \BR2 using the proofs for each link between them:
$\xyInlineRefl{\BL1 \;&\; \BM{1}\arf[l] \;&\; \BR2\arf[l] \;&\; \BM2\arf[l] \;&\; \BL3\arf[l]}$.
...
Notice that these 3 blocks (in chronological order) are required for this recursive PoR to exist: \BM1, \BR2, and \BM2.
...
\begin{equation*}
    \xymatrix@C=1em@R=-.15em{
        ...
        \BL0 & \BL1 & & \BR2 \arff[llu] \arff[ld] & & \BL3 \arff[lu] & \BL4 \arf[ld] \\
        & & \BMtwo1 \arff[lu] & & & \BMtwo2 \arf[llu] \\
    }
\end{equation*}
In this case, \cL is implicitly reflected in \BR2, and \BL3 has two possible recursive PoRs of this: \BL{1} via \BMtwo1; and \BL{0} via \BMone1.
Are both valid?
...
We \emph{can} be sure that \BR2's weight can be fully included in \cL's chain-weight \emph{after} \BL4 is mined (since all PoR paths now exist).
...
This means that \emph{any} PoR path proving that \BR2 reflects \BL1 is worth \emph{at least} as much as \BR2.
...
Thus, we can safely add \BR2's \emph{full} weight via \BL3 (and that weight is attributed to \BL1 since it is the most recent \cL block in \BR2's history).
An attacker would need to produce at least as much work as \BR2 to perform doublespend against \cL \emph{after} \BR2's reflection of \BL1 has been counted (assuming the doublespend competes with \BL1).

Therefore, only the \emph{first} available recursive PoR via \BR2 matters.
The number of paths from \BR2 to past \cL blocks is not relevant: \BL0 is in the history of \BL1, and \BR2 can only count once.
Additionally, the number of paths from \cL blocks to \BR2 is \emph{also} not relevant, since \BR2 is in the history of all such blocks.
...
$\xyInlineRefl{\BL1 \;&\; \BMtwo1 \arf[l] \;&\; \BR2 \arf[l] \;&\; \BMone2 \arf[l] \;&\; \BL3\arf[l]}$.\footnote{
    This recursive PoR path is preferable because it is from the \emph{first possible} \cL block to enable a PoR via \BR2, and goes to the \emph{most recent} \cL block in \BR2's history.
}
We go from \BL3 to \BR2 via \cMone, and from \BR2 to \BL1 via \cMtwo.
...
There could be PoR paths that include multiple blocks from \cMone and \cMtwo (e.g., $\xyInlineRefl{\BL0 \;&\; \BMone1 \arf[l] \;&\; \BMtwo1 \arf[l] \;&\; \BR2 \arf[l] \;&\; \BMone2 \arf[l] \;&\; \BL3 \arf[l]}$).
Whilst these kind of paths are valid, they are not that useful: in the example, we expect that \BMtwo1 already reflects an \cL block, either \BL0 directly or a more recent \cL block.
...
\begin{equation*}
    \xymatrix@R=.3em@C=.75em{
        \BL0 & \BL1 \ar[l] \\
        & \BM0 & \BM1 \ar[l] \arf[ul] & \BM{2} \ar[l] \\
        & & & \BR{1a} \arf[dl] & \BR2 \ar[l] \arf[ul] \arf[ull] \\
        & & \BM{1a} \ar[luu] \arf[dl] & \\
        & \BL{1a} \ar[luuuu]
    }
\end{equation*}

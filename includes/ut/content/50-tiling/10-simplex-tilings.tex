...
Specifically: the pattern must \emph{always} have room for more simplex tiles, and each tile added must, on average, add \emph{more} than 1 new spot for additional tiles.\footnote{
...
\pz{Add that 4 parts are reserved for external reflections, or specify that the rule is to keep only one part for internal reflections if that is the case.}
...
Each tile could even have different valences, too, but this makes designing a tiling more complex.
...
\pz{Unclear at this point why a percentage of the header would correspond to exactly one external reflection.}
...
% \subsection{Tree-Tilings: Preliminary Scaling Analysis}

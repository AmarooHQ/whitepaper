...
\begin{enumerate}
    \item The root tile produces at least \Mroot units of tile-work ($\Mroot \ge 1$).
    \item Each non-root tile is limited to $r^{-d}$ units of tile-work, where $r$ is a network-wide constant and $r > u \ge 2$.
\end{enumerate}
...
At depth $d$, each tile (except the root tile) produces $r^{-d}$ units of tile-work, and there are $u^d$ such tiles.
So at that depth there is at most $u^d r^{-1}$ tile-work produced.

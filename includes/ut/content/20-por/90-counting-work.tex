...
% note: \BR renewed below
\providecommand\xyB[1]{*+[F]+{#1}}

\SelectTips{cm}{}

When chains L and R have the same $B_f^{-1}$ and produce blocks in an orderly fashion, it's trivial to count how much work is contributed by each PoR.
However, real-world blockchains do not produce blocks like this -- sometimes chain L will produce 2 or 3 (or more) blocks before R produces its next one (or vice versa).
\begin{equation*}
    \xymatrix@M=3pt@R=4pt{
        \ar[rrrrrr]_<{Time} & & & & & & \\
        \cdots & \xyB{L_i} \ar[l] & & \xyB{L_{i+1}} \ar[ll] \ar@{.>}[ld] & \xyB{L_{i+2}} \ar[l] & \xyB{L_{i+3}} \ar[l] & \\
        \cdots & & \xyB{R_j} \ar[ll] \ar@{.>}[lu] & & & & \xyB{R_{j+1}} \ar[llll] \ar@{.>}[lu]
    }
\end{equation*}
...
The miner is effectively claiming -- via the absence of a new reflection -- that \BL{2}'s parent (\BL{1}) reflects the most recent R header ($R_j$); that \BL{1} is still \emph{up-to-date}.
From this perspective, it's clear that \BL{2} should contribute reflected weight to the best R headers that were most recently reflected: in this case, just $R_j$.
...
\begin{equation*}
    \xymatrix@M=3pt@R=4pt{
        \ar[rrrrrrr]_<{Time} & & & & & & & \\
        \cdots & \xyB{L_i} \ar[l] & & \xyB{L_{i+1}} \ar[ll] \ar@{.>}[ld] & \xyB{L_{i+2}} \ar[l] & \xyB{L_{i+3}} \ar[l] & & \xyB{L_{i+4}} \ar[ll] \ar@{.>}[ld] \\
        \cdots & & \xyB{R_k} \ar[ll] \ar@{.>}[lu] & & \xyB{R_{k+1}} \ar[ll] & & \xyB{R_{k+2}} \ar[ll] \ar@{.>}[lu]
    }
\end{equation*}
...
The essence of PoR is \emph{the confirmation of another chain's \underline{history}} -- so a reflection cannot contribute to a blocks which only exist \emph{in the reflecting block's \underline{future}}.
So it must be that reflections contribute to the \emph{most recent common ancestor} of all possible L blocks which could include that PoR.
...
% (L_{i+4} \; \ar@{.>}[r] &) \;
Following the arrows: $\xymatrix@C=10pt@M=0pt{R_{k+2} \; \ar@{.>}[r] & \; L_{i+3}}$.
...
Following the arrows: $\xymatrix@C=10pt@M=0pt{R_{k+1} \; \ar[r] & \; R_k \; \ar@{.>}[r] & \; L_{i}}$.
...
Particularly, that the reflected work of \BL{1\cdots 3} should contribute to $R_k$, \emph{not} \BR{1}.

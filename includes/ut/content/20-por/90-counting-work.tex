...
The essence of PoR is \emph{the confirmation of another chain's \underline{history}} -- so a reflection cannot contribute to a block which exist \emph{in the reflecting block's \underline{future}}.
So it must be that reflections contribute to the \emph{most recent common ancestor} of all possible L blocks that could include the PoR.

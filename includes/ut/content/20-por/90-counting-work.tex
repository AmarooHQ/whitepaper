...
\begin{equation*}
    \xyMChains{
        ...
        \xyL{\text{Chain R}} & \cdots & & \xyB{R_j} \ar[ll] \ar@{.>}[lu] & & & & \xyB{R_{j+1}} \ar[llll] \ar@{.>}[lu] \\
        \xyTimeHoriz{rrrrrrr} & & & & & & & \\
    }
\end{equation*}
Should \BRj{1} include 3 PoRs, or just 1?
How should \BRj{1} calculate the weight of those PoRs and the total weight contributed to chain R?

If \BRj{1} had reflected only \BL{1}, then it's clear that we would count work from both \BL{1} and \BRj{1} --- that much is easy.
However, \BRj{1} reflected \emph{three} L headers.
Should \BRj{1} count work from \emph{all three}, or something else?
Why?
...
So, yes \BRj{1} should include and count work from all 3 PoRs in this case.
...
\pz{Unclear, did you mean \BL{4} instead of \BL{3}?}

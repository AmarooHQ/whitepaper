...
When chains L and R have the same $B_f^{-1}$ and produce blocks in an orderly fashion, it's trivial to count how much work is contributed by each PoR. However, real-world blockchains do not produce blocks like this -- sometimes chain L will produce 2 or 3 (or more) blocks before R produces its next one (or vice versa).
\begin{equation*}
    \xymatrix@M=4pt@R=10pt{
        \ar[rrrrrr]_<{Time} & & & & & & \\
        \cdots & L_i \ar[l] & & L_{i+1} \ar[ll] \ar@{-->}[ld] & L_{i+2} \ar[l] & L_{i+3} \ar[l] & \\
        \cdots & & R_j \ar[ll] \ar@{-->}[lu] & & & & R_{j+1} \ar[llll] \ar@{-->}[lu]
    }
\end{equation*}
...
\begin{equation*}
    \xymatrix@M=4pt@R=10pt{
        \ar[rrrrrrr]_<{Time} & & & & & & & \\
        \cdots & L_i \ar[l] & & L_{i+1} \ar[ll] \ar@{-->}[ld] & L_{i+2} \ar[l] & L_{i+3} \ar[l] & & L_{i+4} \ar[ll] \ar@{-->}[ld] \\
        \cdots & & R_k \ar[ll] \ar@{-->}[lu] & & R_{k+1} \ar[ll] & & R_{k+2} \ar[ll] \ar@{-->}[lu]
    }
\end{equation*}
...
This leads us to a simple and elegant way to count reflected work: follow the arrows back to the most recently reflected local block.

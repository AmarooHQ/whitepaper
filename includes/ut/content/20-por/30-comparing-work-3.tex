...
\textbf{Here's the problem:} \emph{universal and generic} conversion between qualitatively different units is impossible.\footnote{
    ...
    For relevant background, see his article \href{https://criticalfallibilism.com/multi-factor-decision-making-math}{Multi-Factor Decision Making Math}.
}
...
We already know some background context, like that we're using blockchains, that root tokens are involved (which are exact and fungible), that nodes have access to information that's provable or that's already in their state, etc.

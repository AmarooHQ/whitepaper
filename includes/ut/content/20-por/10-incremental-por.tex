...
I also wrote a simplified implementation of this method for Bitcoin SPV proofs in the very early days of Ethereum\footnote{
    \url{\citeCopprChainheadersLink} \href{https://web.archive.org/web/20240927044057/https://raw.githubusercontent.com/XertroV/coppr/refs/heads/master/chainheaders.py}{[m1]}
} --- a precursor to the later-successful BTC Relay\footnote{
    \url{\citeBTCRelayLink}
...
\defineTermTex{Projection}{
  ...
  For example, \href{\citeBTCRelayLink}{BTC Relay} is a smart contract by which Ethereum previously hosted a \emph{projection} of Bitcoin.
  The \emph{act} of one chain creating and maintaining the projection of another is called \emph{imaging}
}

\subsection{A Projection of Bitcoin in Ethereum}
...
Since Bitcoin's consensus and PoW algorithm is simple and has low overhead, implementing the necessary logic as an Ethereum SC is viable.
...
\subsection{Incrementally Implementing \emph{Proof of Reflection}}

\label{sec:two-blockchains}

Let's consider two hypothetical and distinct blockchains --- chain \cL and chain \cR.
To begin with, you can imagine these as Bitcoin and Ethereum (before migrating to PoS).
However, the changes that are required by \emph{Proof of Reflection} would require major changes to an existing chain's consensus mechanism, so it's unlikely that changes like these would ever be integrated.
...
\subsubsection{Step 1. Chain \cR Images Chain \cL}
...
\subsubsection{Step 2. \cL Images \cR}

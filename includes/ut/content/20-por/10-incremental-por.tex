...
Using these proofs, we can share security between these two chains, such that attacking either is as difficult as attacking both.

Our starting point is the question: is there anything \emph{in principle} that prohibits blockchains sharing security?
We already have some examples of this (e.g., merged mining), but those existing methods aren't general.
Can we come up with a general to share security between chains?
...
This isn't a new idea: in 2013\footnote{
    \url{https://bitcointalk.org/index.php?topic=198032.0},
    \url{https://bitcointalk.org/index.php?topic=598784.0}
}, I proposed a system which used this method to support rich cross-chain exchange.
I also wrote a simplified implementation of this method for Bitcoin SPV proofs in the very early days of Ethereum\footnote{
    \url{https://github.com/XertroV/coppr/blob/master/chainheaders.py}
...
In lieu of an existing term, let's call it a ``projection'', and let's call the \emph{act} of one chain creating a projection of another ``imaging''.
...
The typical answer is to enables proofs of some state or that some transactions occurred on the imaged chain.

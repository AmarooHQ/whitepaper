% Use this file to enable autocompile / autoreload type features with vscode et al.

%
% LOTS OF STUFF FROM PANDOC
%

\PassOptionsToPackage{unicode=true}{hyperref} % options for packages loaded elsewhere
\PassOptionsToPackage{hyphens}{url}
%
\documentclass[a4paper]{article}
\usepackage{lmodern}
\usepackage{pifont}
\usepackage{amsfonts}
\usepackage{amssymb,amsmath}
\usepackage{ifxetex,ifluatex}
\usepackage{fixltx2e} % provides \textsubscript
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi=0 % if pdftex
  \usepackage[T1]{fontenc}
  \usepackage[utf8]{inputenc}
  \usepackage{textcomp} % provides euro and other symbols
\else % if luatex or xelatex
  \usepackage{unicode-math}
  \defaultfontfeatures{Ligatures=TeX,Scale=MatchLowercase}
\fi
% use upquote if available, for straight quotes in verbatim environments
\IfFileExists{upquote.sty}{\usepackage{upquote}}{}
% use microtype if available
\IfFileExists{microtype.sty}{%
\usepackage[]{microtype}
\UseMicrotypeSet[protrusion]{basicmath} % disable protrusion for tt fonts
}{}
\IfFileExists{parskip.sty}{%
\usepackage{parskip}
}{% else
\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt minus 1pt}
}
\usepackage[unicode]{hyperref}
\hypersetup{
            pdftitle={test document},
            pdfauthor={Max Kaye},
            pdfborder={0 0 0},
            breaklinks=true}
\urlstyle{same}  % don't use monospace font for urls
\usepackage[left=3cm,right=3cm,top=3cm,bottom=3cm]{geometry}
\usepackage{longtable,booktabs}
% Fix footnotes in tables (requires footnote package)
\IfFileExists{footnote.sty}{\usepackage{footnote}\makesavenoteenv{longtable}}{}
\setlength{\emergencystretch}{3em}  % prevent overfull lines
\providecommand{\tightlist}{%
  \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}
\setcounter{secnumdepth}{5}
% Redefines (sub)paragraphs to behave more like sections
\ifx\paragraph\undefined\else
\let\oldparagraph\paragraph
\renewcommand{\paragraph}[1]{\oldparagraph{#1}\mbox{}}
\fi
\ifx\subparagraph\undefined\else
\let\oldsubparagraph\subparagraph
\renewcommand{\subparagraph}[1]{\oldsubparagraph{#1}\mbox{}}
\fi

% set default figure placement to htbp
\makeatletter
\def\fps@figure{htbp}
\makeatother

%
% ALL THE PACKAGES
%

\usepackage{lmodern}
\usepackage{amssymb}
\usepackage[left=3cm,right=3cm,top=3cm,bottom=3cm]{geometry}
\usepackage{longtable,booktabs}
\usepackage[utf8]{inputenc}
\usepackage{comment}
\usepackage[lastpage,user]{zref}
\usepackage{fancyhdr}
\usepackage[dvipsnames]{xcolor}
\usepackage{amsmath,amsfonts,bm}
\usepackage{mathtools}
\usepackage{mdframed}
\usepackage[skins,breakable,most]{tcolorbox}
\usepackage[yyyymmdd,hhmmss]{datetime}
\usepackage{nicefrac}
\usepackage{epigraph}
\usepackage{verbatim}
\usepackage{subcaption}
\usepackage{float}
\usepackage{xhfill}
\usepackage{varwidth}
\usepackage{pdfpages}
\usepackage{algpseudocode}
\usepackage{algorithm}
\usepackage[automake,toc,nogroupskip]{glossaries}
\usepackage[titles]{tocloft}
\usepackage[fit]{truncate}
\usepackage{wallpaper}
\usepackage[section]{placeins}
\usepackage[export]{adjustbox}
\usepackage{ifthen}
\usepackage{array}
\usepackage{tabularx}
\usepackage{ctable}
\usepackage{trimspaces}
\usepackage{cancel}
\usepackage[all,cmtip]{xy}
\usepackage{morewrites}
\usepackage{xparse}
\usepackage{forest}
\usepackage{mathdots}
%% for labeling lists
\usepackage{enumitem}
%% use cref not autoref for items
\usepackage[nameinlink]{cleveref}

\usepackage{marginnote}
\usepackage{booktabs}
\usepackage{chngpage}
\usepackage{etoolbox}

\graphicspath{{../../../../}{../../../../diags/}}


\providecommand*\ErrorOnReleaseIfTODOs{no}
\providecommand*\BuildMode{draft}
\providecommand*\preparedfor{}

\providecommand*\wordcount{99-test-99}

\makeatletter
\def\input@path{{../../../}{../../../../}{../../../../diags/}{../../../../../}{../../../../../../}}
\makeatother


\input{includes/ut/00-preamble/000-all-preambles.tex}

\makeatletter
\providecommand\defineTermTex[2]{
  \begin{tcolorbox}[]
      \textbf{#1}: \trim@post@space{#2}.
  \end{tcolorbox}
    % GLOSSARY ENTRY: #1: #2.
}
\makeatother

\providecommand\todo[1]{
    TODO: #1
}

\providecommand\todoDraftOnly[1]{
    TODO DRAFT ONLY: #1
}


\begin{document}

\tableofcontents

\newpage

\begin{section}{Abstract}

\input{../00-abstract.tex}

\end{section}

\newpage

\section{Constructing UT}

\subsection{UT2 Dapp-chains}

\subsubsection{PoS Dapp-chains}

\input{25-constructing-ut/45-dapp-chains-extra-short.tex}


\newpage

\section{Practical Considerations}

\begin{subsection}{Expedited Transactions}

  \input{27-practical/97-10-expedited-transactions.tex}

\end{subsection}



\subsection{Initial Configuration}

\label{sec:initial-config}

\input{27-practical/99-50-initial-config.tex}


\section{Practical Consid.}

\subsection{1}
\subsection{2}
\subsection{3}
\subsection{4}
\subsection{5}
\subsection{6}
\subsection{7}
\subsection{8}
\subsection{9}
\subsection{10}
\subsection{11}
\subsection{12}
\subsection{13}

\subsection{Dapp-Chain PoS Design}

\label[Dapp-Chain PoS Design]{sec:pos-dapp-chain-design}

\subsubsection{Speaker-Listener Faults}

label[PoS Design: Speaker-Listener Faults]{sec:pos-dapp-chain-design:speaker-listener}


\begin{section}{Cost of PoS section}

  \input{20-por/99-cost-of-pos.tex}

\end{section}

\newpage

  \begin{section}{Section 1 part 2}
%   \DeclareAxiom{MaximalReflection}{%
%   A miner should reflect all possible blocks.
% }{%
%   A block, \BL{}, is invalid unless, for each reflected block \BR{}:
%   \begin{axiomreqs}

%       \item \BR{} is valid under this Axiom; and
%       \item each block that was reflected by \BR{} or is a parent of \BR{}:
%       \begin{itemize}
%           \item is reflected by \BL{}; or
%           \item is an ancestor of \BL{}; or
%           \item was reflected by an ancestor of \BL{}.
%       \end{itemize}
%   \end{axiomreqs}
% }[%
%   $V(\BL{}) =
%   % \\ \text{\hspace{5.5em}}
%   V(\BR{}) \land (
%       (\BL{} \refls{} \BBj{})
%       \lor (\BL{} \descendsFrom \BBj{})
%       \lor (\exists b (\BL{} \descendsFrom b \refls{} \BBj{}))
%   ) \\
%   \text{\hspace{7.5em}}
%   \forall \{(\BR{}, \BBj{}) : \BL{} \refls \BR{}
%       \land (\BR{} \refls{} \BBj{} \lor \BR{} \childOf \BBj{})\}
%   $
% ]

% \subsection{PoR}

\input{20-por/10-incremental-por.tex}

% \subsubsection{SRT}

\input{includes/ut/content/20-por/40-single-root-token-2.tex}

\subsection{Constructing uT}


\input{includes/ut/content/25-constructing-ut/20-scaling-complexity-intuition.tex}



\input{includes/ut/content/25-constructing-ut/60-going-further-research.tex}


\subsection{Practical}

% \input{includes/ut/content/27-practical/10-refl-availability.tex}

% \input{includes/ut/content/27-practical/25-stateless-and-fraud-proofs.tex}

% \input{includes/ut/content/27-practical/30-the-PoR-graph.tex}


% \input{27-practical/50-dags-00-all.tex}


% \input{27-practical/50-dags-25-dos-attacks.tex}
% \input{27-practical/50-dags-50-merging-histories.tex}
% \input{27-practical/50-dags-60-nipoppows.tex}
% \input{27-practical/50-dags-75-interaction-with-por-graph.tex}

\subsection{SPV}

\input{27-practical/95-spv-2-mk}

% \input{27-practical/98-ut1-protocol-summary.tex}

\subsection{Complexity}

% \input{30-complexity/70-bandwidth-complexity.tex}

\subsection{UT Aleph}

\input{50-tiling/00-tiling-all}

\clearpage

\section{Conclusion}

\input{80-conclusions/20-addressing-trilemma}

%
% The in-prog doc we want to preview
%

% \input{27-practical/90-simplex-security-and-cec.tex}
% \input{20-por/90-counting-work.tex}

\end{section}

\appendix

% \input{99-appendix/20-cec-iteration.tex}

\end{document}

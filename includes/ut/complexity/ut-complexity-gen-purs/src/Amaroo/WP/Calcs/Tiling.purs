...
import Amaroo.WP.Calcs (mkSimplePs, utChainCalc, UtOptimizations, utScaleOptionsFmt)
...
-- opt
ut_params :: UtOptimizations
ut_params = {explicitPoRs: true, headerOmission: true, hashTruncation: true, onlyNecessaryHeaders: false}
ut_params_hot :: UtOptimizations
ut_params_hot = {explicitPoRs: false, headerOmission: true, hashTruncation: true, onlyNecessaryHeaders: false}
...
tree_tiling_table :: Int -> {k :: Number, lp :: Boolean} -> UtOptimizations -> Table
tree_tiling_table v {k, lp} scaling_params = Table
    headings
    ({md: mkSpacer <$> A.replicate (l) 3, texTabular: "l" <> repeatSafe (l-1) "r"})
    $ mkRow <$> row_hs
  where
    row_hs = flip (filter_big_hs_for_v) v $ if lp then [0, 1, 2, 5, 10, 15, 20, 30, 60] else A.range 0 6 <> [10, 15, 20, 25, 30]
    headings = (if lp then trim_for_lp else id)
      [ if lp then "# of Layers" else "$d$"
      -- , "$N_{\\text{tiles}|h}$"
      , "$N_{\\text{tiles}}$"
      , "$\\Sigma N_1$"
      , "$\\Sigma N_2$"
      ...
      , "$\\Sigma \\text{TPS}_2$"
      , "$\\mathbb{C}^\\prime$"
      , "$\\times \\UT{\\text{"<>ut_var_name<>"}}$"
      ]
    ut_var_name = utScaleOptionsFmt scaling_params
    l = A.length headings
    -- s_tps1, x_ut_tps1
    mkRow d = (if lp then trim_for_lp else id) $
      ...
      where
        ...
        ut_cs = utChainCalc (params_tt k) scaling_params
        ut_n1 = ut_cs.d1.n
        ut_n2 = ut_cs.d2.n
        -- tile_chain_prop = if d
...
tree_tiling_3k_v3_table = tree_tiling_table 3 {k: 3000.0, lp: false} ut_params

tree_tiling_3k_v3_table_hot :: Table
tree_tiling_3k_v3_table_hot = tree_tiling_table 3 {k: 3000.0, lp: false} ut_params_hot

tree_tiling_3k_v3_table_lp :: Table
tree_tiling_3k_v3_table_lp = tree_tiling_table 3 {k: 3000.0, lp: true} ut_params

tree_tiling_3k_v4_table :: Table
tree_tiling_3k_v4_table = tree_tiling_table 4 {k: 3000.0, lp: false} ut_params

tree_tiling_3k_v5_table :: Table
tree_tiling_3k_v5_table = tree_tiling_table 5 {k: 3000.0, lp: false} ut_params

tree_tiling_20k_v3_table :: Table
tree_tiling_20k_v3_table = tree_tiling_table 3 {k: 20000.0, lp: false} ut_params

tree_tiling_20k_v4_table :: Table
tree_tiling_20k_v4_table = tree_tiling_table 4 {k: 20000.0, lp: false} ut_params

tree_tiling_20k_v5_table :: Table
tree_tiling_20k_v5_table = tree_tiling_table 5 {k: 20000.0, lp: false} ut_params

...
If we are to compare (and convert) work between PoW chains, then we \emph{must} be able to convert between L-hashes and R-hashes \emph{in a consistent way}.
Consistency, here, means the conversion shouldn't change the outcome of the fork rule;
if the fork rule said that $L_{i \cdots j,a} > L_{i \cdots j,b}$ when using L-hashes as the units, then it \emph{should give the same result} for \emph{all} cases when the units are changed to R-hashes.
...
Since both weight and hashes can be \emph{summed}, an easy way for this to work is for us to use a \emph{linear} conversion method (i.e., the units are proportional and we have some \emph{constant of conversion} for that specific situation).
That doesn't require that \emph{mid-states} are linearly convertible, though.
...
\aside{
    If we can measure the \emph{rate of work} in some direct way (e.g., number of hashes) then we can also \emph{scale block rewards according to both the network difficulty and the demonstrated rate of work}.
...
\textbf{Here's the problem:} \emph{universal and generic} conversion between qualitatively different units is impossible.\footnote{See \href{https://criticalfallibilism.com/multi-factor-decision-making-math}{Multi-Factor Decision Making Math} and \href{https://curi.gumroad.com/l/mhtbA}{Critical Fallibilism Course}, both by Elliot Temple.}
...
The goal of this conversion is the only thing we know right now: to convert chain-work so that \emph{PoR works}.
...
In order to discuss \emph{how} to convert (i.e., the ideas themselves) we need to know the \emph{context} that we operate in.
We know some of the context already: the rest of the world, e.g., we know about some ways to attack blockchains.
However, that's just background context.
What specific contexts can help us convert?
Here are two:
...
\begin{comment}
<!-- It's also partially covered in (or related to) [Elliot's *Yes or No Philosophy* course](https://gumroad.com/l/hxqsh) and some of his articles, e.g., [*IGCs* ({Idea, Goal, Context} triples)](https://curi.us/2387-igcs) and [*Bottleneck Examples*](https://curi.us/2353-bottleneck-examples). -->

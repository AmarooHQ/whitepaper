...
\begin{tikzpicture}[node distance=1cm]
    ...
    \node (ethChain) [chainName, right of=btcChain, xshift=4.5cm] {Ethereum};
    ...
    \foreach \x in {1,...,3} {
        ...
        % btc state
        \draw [braceL] (btc\x.west |- btc\x.north) --
            node (btcState\x) [anchor=east, xshift=-10pt, text width=130pt,align=right] {\footnotesize{
                \textsc{STATE} \\ Known Headers: $ETH_{0 \cdots 40j+\ethLastEnd}$ \\ Proof of Reflection: $ETH_{40j+\ethLastEnd}$}}
        ...
        \node (ethS\x) [block, below = of \prevNode |- \oPrev, yshift=\ys] {$\MakeUppercase{eth}_{40j+\ethBStart}$};
        \draw [arrowParent] (ethS\x) -- (\prevNode);

        \node (ethDots\x) [dots, below = of ethS\x, yshift=0.5cm] {\vdots};
        \draw [arrowParent] (ethDots\x) -- (ethS\x);

        \node (ethE\x) [block, below = of ethDots\x, yshift=0.5cm] {$\MakeUppercase{eth}_{40j+\ethBEnd}$};
        \draw [arrowParent] (ethE\x) -- (ethDots\x);

        \draw [braceR] (ethS\x.east |- ethS\x.north) --
            node (ethStateCoord\x) [anchor=west, xshift=10pt, text width=120pt] {\footnotesize{\textsc{STATE} \\ Known Headers: $BTC_{0 \cdots i+\x}$ \\ Proof of Reflection: $BTC_{i+\x}$}}

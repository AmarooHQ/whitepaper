...
\begin{tikzpicture}[node distance=.8cm]
    ...
    \node (a2) [action] at (a1 |- f2) {Unreadable; \\ Discard};
    \node (a3) [action] at (a1 |- f3) {Invalid; \\ Use w/ FP};
    ...
    \draw [braceUnder] (BL1) -- node (complexity1) [lb, bigText] {$O(c)$} (divPointB);
    \draw [braceUnder] (divPointB) -- node (complexity2) [lb, bigText] {$O(c^2)$} (BR);

    \draw [stealth-stealth] (complexity1) -- node [label] {Independent \\ Progression} (complexity2);
\end{tikzpicture}%
\end{document}%

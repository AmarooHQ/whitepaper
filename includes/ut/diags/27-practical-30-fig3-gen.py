...
out_file = '27-practical-30-fig3-refls.tikz'
...
def seg_is_suitable(cbs: list[list[int]]):
    for c, chain in enumerate(cbs):
        if sum(chain) == 0 or (c != 5 and (chain[0] or chain[-1])):
            return False
    return len(cbs) > 1

def gen_chain_blocks():
    return list(list(sum([(1 if random.random() < prob_mine else 0) for _ in range(1)]) for _ in range(n_ticks)) for _ in range(n_chains))

chain_blocks = []
while not seg_is_suitable(chain_blocks):
    chain_blocks = gen_chain_blocks()
    chain_blocks[focus_chain] = [1] + [0] * (n_ticks - 2) + [1]
...
def refl_tips_of(c, t):
    rts = set((c2, t-1) for c2 in range(n_chains) if c2 != c and chain_blocks[c2][t-1])
    if len(rts) == 0:

...
\tikzstyle{refl} = [line width=2pt, shorten >= 3pt, shorten <= 3pt]%
\tikzstyle{splxNode} = [draw=black,shape=circle]%
%
\newcommand\TikzSimplex{
    \pgfmathsetmacro\rOffset{(360/\SimplexChains)+90}
    ...
    \foreach \i in {1,...,\SimplexChains} {
            ...
            \node (\SimplexNodeName//\i)
                [splxNode, fill=\SimplexNodeColor, inner sep=\SimplexNodeRadius]
    ...
    \foreach \i in {1,...,\SimplexChains} {
            \foreach \j in {1,...,\SimplexChains} {
                    \ifnum \i>\j
                        \draw [refl] (\SimplexNodeName//\i) -- (\SimplexNodeName//\j);

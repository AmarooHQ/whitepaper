 ...
 \begin{subsection}{Prerequisites for PoR}
     ...
     \begin{enumerate}
         ...
         \item \label{item:headers-a-in-b}
               The headers of \(C_A\) can be (and are) recorded --- promptly and unambiguously --- in \(C_B\);
         \item \label{item:headers-b-in-a}
               The headers of \(C_B\) can be (and are) recorded --- promptly and unambiguously --- in \(C_A\); and
...
\begin{section}{The Simplex}
 ...
 Additional details are in s3.2 and s3.3 of the whitepaper. This configuration natively has $O(c^2)$ scalability, though this can be improved to $O(c^3)$ or better if the simplex-chains host child blockchains (a.k.a., dapp-chains).
 ...
 \begin{subsection}{Method of Construction}
    ...
    A blockchain is started from a genesis block. The simplex can be started from one or more genesis blocks (in the case of 1 genesis block, the simplex-chain spawns a new simplex-chain to create an, initially, 2-chain simplex). As simplex-chains approach maximum capacity, new simplex-chains are instantiated. There are many different approaches one could take to design methods of instantiation based on particular requirements and/or features that are desired.

 \end{subsection}
 \begin{subsection}{Method of Operation}

    Miners that are producing blocks for simplex-chains behave slightly differently to normal. They are required to reflect the headers of other simplex-chains, and thus must include them in their blocks. However, for the smoothest operation of the simplex, each miner should download *all* other blocks of other simplex-chains; this ensures that only existent blocks get reflected.

    Futhermore, miners should partially validate blocks from other simplex-chains, but only those segments of the block that deal with reflections. This additional processing (which is not overly burdensome on the miner) ensures that proofs of reflection (e.g., merkle branches) do not need to be created or transmitted (this was hinted at previously). This improves overall throughput and reliability. It is technically optional, though other methods for ensuring the integrity are then required.
...
\begin{section}{Simplex-Tiling}
 ...
 \begin{subsection}{Requirements for Participating Simplex-tiles}
     ...
     Simplex-tiles should reserve \nicefrac{1}{8} of their capacity for internal reflections, and \nicefrac{3}{8}\footnote{Simplex-tiles have a property called \emph{valence} which determines how many other simplex-tiles they can do external reflections with. A valence of 3 is the minimum required for a scalable tiling, but smaller and larger valences are possible. The ratios of capacity that a simplex-tile should reserve for internal and external reflections is based on its valence.} of their capacity for \emph{external} reflections, i.e., mutual reflections with \emph{other} tiles. See \autoref{fig:simplex-tiling}. Note: there are additional patterns of tiling. An example is shown in \autoref{fig:alt-tiling}.
 ...
 \begin{subsection}{Utility}
     ...
     $O(n)$ scalability, i.e., \emph{unbounded capacity}.

     Further improved security such that attacking one chain in the tiling is as difficult as attacking the complete tiling (i.e., all participating simplex-chains).
